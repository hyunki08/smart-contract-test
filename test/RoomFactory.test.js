...
describe("RoomFactory", function () {
    ...
    describe("createRoom", function () {
        ...
        it("should create a room", async function () {
            const receipt = await this.roomFactory.createRoom({ from: roomOwner1, value: amount });
        ...
        it("should emit a RommCreated event", async function () {
            const receipt = await this.roomFactory.createRoom({ from: roomOwner1, value: amount });
            const event = await expectEvent(receipt, "RoomCreated");
        ...
        it("can create multiple rooms", async function () {
            const receipt1 = await this.roomFactory.createRoom({ from: roomOwner1, value: amount });
            const event1 = await expectEvent(receipt1, "RoomCreated");
            const receipt2 = await this.roomFactory.createRoom({ from: roomOwner2, value: amount });
            const event2 = await expectEvent(receipt2, "RoomCreated");
            const receipt3 = await this.roomFactory.createRoom({ from: roomOwner3, value: 0 });
        ...
        it("can accept an empty deposit", async function () {
            const receipt = await this.roomFactory.createRoom({from: roomOwner1, value: web3.utils.toWei("0", "ether")});
            const event = await expectEvent(receipt, "RoomCreated");
            const factoryBalance = await web3.eth.getBalance(this.roomFactory.address);
            const roomBalance = await web3.eth.getBalance(event.args._room);
